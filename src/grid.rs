...
pub fn perform_step(grid: &Vec<Vec<u8>>) -> Vec<Vec<u8>> {
    ...
    for i in 0..grid.len() {
        for j in 0..grid[0].len() {
            if j == 0 {
                update[i][j] = rules::rule_30_u8(to_u8(&[0, grid[i][0], grid[i][1]]));
            } else if j == (grid[0].len() - 1) {
                update[i][j] = rules::rule_30_u8(to_u8(&[grid[i][j - 1], grid[i][j], 0]));
            } else {
                update[i][j] = rules::rule_30_u8(to_u8(&grid[i][(j - 1)..(j + 2)].to_vec()));
            }
        }
    }
    update
...
pub fn cmp_next(lattice: &Vec<u8>) -> Vec<u8> {
    /* Computes the next lattice. */
    let mut update = lattice.clone();
    for i in 0..lattice.len() {
        if i == 0 {
            update[i] =
                rules::rule_90_u8(to_u8(&[*lattice.last().unwrap(), lattice[0], lattice[1]]));
        } else if i == (lattice.len() - 1) {
            update[i] = rules::rule_90_u8(to_u8(&[lattice[i - 1], lattice[i], lattice[0]]));
        } else {
            update[i] = rules::rule_90_u8(to_u8(&lattice[(i - 1)..(i + 2)].to_vec()));
        }
    }
    update
}
